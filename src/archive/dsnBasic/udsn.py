...
def GetLabeledImage(G, thresh, W, H, feature):
    LG = G.copy()    
    LG.remove_edges_from([(u,v) for (u,v,d) in  LG.edges(data=True) if (d[feature] < thresh)])        
...
def GetGraphImages(G, W, H, feature):
    imgX = np.zeros((W, H), np.single)
    imgY = np.zeros((W, H), np.single)
    
    for u, v, d in G.edges(data = True):
        if u[0] == v[0]:
            imgX[u[0], u[1]] = d[feature]
        else: 
            imgY[u[0], u[1]] = d[feature]
...
def ApplyDSN(netOut):    
    ...
    for u, v, d in G.edges(data = True):
        d['weight'] =  -abs(npGXY[u[0], u[1]] - npGXY[v[0], v[1]])
    
    #[WG, CG] = ApplyDSNGraph(G)
    #wsImg, ccImg = GetSegImages(WG, CG, W, H)
    return (W, H, G)

def MinMaxWatershedCut(G):

    for u, v, d in G.edges(data = True):        
        #print('edges for: ' + str(u))
        max1 = -math.inf
        max2 = -math.inf
        for uev in G[u]:
            if uev != v:
