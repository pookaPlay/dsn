...
from QuanSynData import GetData
...
if __name__ == '__main__':
    ...
    for yi in range(YZ.shape[0]):
        YZ[yi,:,:] = FixZeroLabels(YZ[yi,:,:])

    XT = torch.tensor(X, requires_grad=False)                
    YT = torch.tensor(YZ, requires_grad=False)                                                
    ...
    model = Linear(3, 1, 3, True).to(device)
    ...
    for cepoch in range(0,numEpochs):
        ...
        with torch.enable_grad():                            
            ...
            if ((cepoch % logScores) == 0): 
                ...
                vname = "train_" + str(saveNum) + "_final.pkl"
                with open(vname, 'wb') as f:
                    pickle.dump(trn_rerror_lst, f)            

                vname = "train_" + str(saveNum) + "_" + str(cepoch) + ".pkl"
                with open(vname, 'wb') as f:
                    pickle.dump(trn_rerror_lst, f)            
