	...
	const vector<double>&		edges)
{
	...
	for( int n = 0; n < numseeds; n++ )
	{
		...
		if(storeind != oind)
		{
			kseedsx[n] = static_cast<double>(storeind%m_width);
			kseedsy[n] = static_cast<double>(storeind / m_width);
			kseedsl[n] = static_cast<double>(m_lvec[storeind]);
			kseedsa[n] = static_cast<double>(m_avec[storeind]);
			kseedsb[n] = static_cast<double>(m_bvec[storeind]);
		}
	}
}
	...
	const vector<double>&		edgemag)
{
	...
	{
		int ye = y*static_cast<int>(yerrperstrip);
		for( int x = 0; x < xstrips; x++ )
		{
			int xe = x*static_cast<int>(xerrperstrip);
	...
	const vector<double>&		edgemag)
{
	...
	{
		int Y = y*static_cast<int>(step) + yoff;
		if( Y > m_height-1 ) break;

		for( int x = 0; x < m_width; x++ )
		{
			//int X = x*step + xoff;//square grid
			int X = x*static_cast<int>(step) + (xoff << (r & 0x1));//hex grid
	...
	const int&					NUMITR)
{
	...
	//----------------
	int offset = STEP;
	if (STEP < 10) offset = static_cast<int>(static_cast<double>(STEP)*1.5);
	//----------------
	...
	const double&				m)
{
	...
	int newsz = static_cast<int>(double(sz) / double(STEP*STEP)); 
	EnforceLabelConnectivity(klabels, m_width, m_height, nlabels, numlabels, newsz);
	...
	const double&				m)//weight given to spatial distance
{
	...
	int STEP = static_cast<int>(sqrt(double(sz)/double(K)) + 2.0);//adding a small value in the even the STEP size is too small.
