...
void OptimizerState::UpdateDir() {
	...
	if (_isnan(dir[0])) {
		cerr << "\nL-BFGS failed so going with steepest\n" << endl;
		dir = steepestDescDir;
	}
	...
	if (origDirDeriv >= 0) {
		cerr << "\nL-BFGS chose a non-descent direction so going with steepest\n" << endl;
		dir = steepestDescDir;
	} //else cerr << "\nL-BFGS chose a descent direction: " << origDirDeriv << endl;
...
void OptimizerState::TestDirDeriv() {
	...
	double eps = 1.05e-6 / dirNorm;
	GetNextPoint(eps);
	double val2 = EvalL1();
	double numDeriv = (val2 - value) / eps;
	double deriv = DirDeriv();
	if (!quiet) 
		cout << "  Grad: " << numDeriv << " vs. " << deriv << " \n";
}
...
void OptimizerState::BackTrackingLineSearch() {
	...
	// The most likely reason for this is a bug in your function's gradient computation
	if (origDirDeriv >= 0) {
		cerr << "\nL-BFGS chose a non-descent direction: check your gradient: " << origDirDeriv << endl;
		func.Debug(newX, newGrad);
		exit(1);
	} //else cerr << "\nL-BFGS chose a descent direction: " << origDirDeriv << endl;
