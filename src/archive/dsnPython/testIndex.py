...
def DirGrad(img):
    ...
    dy[:, :, -1, :] = 0
    #dyy = torch.pow(dy, 2)
    #GXY = torch.sqrt(torch.pow(dx,2)+ torch.pow(dy,2))
    GXY = torch.sqrt(torch.abs(dx)+ torch.abs(dy))
            
    #dxout = F.max_pool2d(dxx, (2,2))
