...
def ApplyDSNGraph(G):
    ...
    # pointer cycle to kepe track of watershed nodes
    wsNextNodes = dict()    
    ...
    for u, v, w in sortedEdges:
        # new basin
        if ((WG.nodes[u]['label'] == 0) and (WG.nodes[u]['label'] == 0)): 
            WG.nodes[u]['label'] = labelUpto
            WG.nodes[v]['label'] = labelUpto
            CG.add_node(labelUpto, weight = w)
            # pointer cycle 
            wsNextNodes[labelUpto] = dict()
            wsNextNodes[labelUpto][u] = v
            wsNextNodes[labelUpto][v] = u            
            # increment baisn count
...
def EvalDSN(G, nlabels_dict, W, H, numInputs):
    ...
    numWSBasins = labelUpto-1
    if verbose:
        print("Watershed has " + str(numWSBasins) + " basins")

    ##############################################
    ## Setup for Correlation Clustering Threshold    
    ...
    if numWSBasins > 1:
        ...
        # apply predefined threshold
        if numWSBasins > 9:
            thresholdi = int(len(ccSorted)*0.75)
        else:
            thresholdi = int(len(ccSorted)*0.5)
        threshold = ccSorted[thresholdi][2]    
        ccThresh = [ [d[0], d[1], threshold - d[2]] for d in ccSorted]
        #print("CCThresh is " + str(ccThresh[0]) + " -> " + str(ccThresh[-1]) )
