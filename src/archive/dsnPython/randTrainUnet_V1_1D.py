...
def GetRandWeights(xPred, nodeLabels):
    ...
    nlabels_dict = dict()
    for u, v, d in G.edges(data = True):
        d['weight'] =  xPred[u[0], u[1]] + xPred[v[0], v[1]]               
...
def RandLossV1(uOut, yones, nodeLabels):
...
y = torch.ones([1, 1, XS.shape[2], XS.shape[3] ], dtype=torch.float32)
...
img = GetConnectedComponentImage(npXPred, .0)
plt.figure(5)
plt.imshow(img)
