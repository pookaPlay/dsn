...
def RandLossV2(uOut, nodeLabels, G):
    ...
    [xCounts, yCounts, bestT, totalPos, totalNeg] = GetRandWeights(G, nodeLabels)
...
def GetWatershedCut(xPred, yPred, W, H):
    ...
    for (u,v,d) in G.edges(data = True):           
        ...
        if minMaxU[0] == minMaxV[0]:    #  vertical edges
            d['minmaxChannel'] = 1
        else:
            d['minmaxChannel'] = 0
...
def ApplyWatershedCut(G, uOut):
    ...
    wsOut = torch.zeros([1, 2, uOut.shape[2], uOut.shape[3] ], dtype=torch.float32)
    
    for (u,v,d) in G.edges(data = True):
        ch = d['channel']
        mm = d['minmaxChannel']
        wsOut[0,ch,u[0],u[1]] = uOut[0,ch,u[0], u[1]] - uOut[0,mm,d['minmax'][0][0], d['minmax'][0][1] ]
...
for epoch in range(epochs):
    ...
    G = GetWatershedCut(npXPred, npYPred, W, H)
