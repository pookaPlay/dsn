...
def get_train_img_stack(pth, epth, num_stacks):
    ...
    for idx in range(0,num_stacks):
        img_paths = img_paths + glob.glob(pth+'/stack_'+str(idx+20)+'_5_3.tif')
        img_pathsA = img_paths + glob.glob(pth+'/stack_'+str(idx+19)+'_5_3.tif')
        img_pathsB = img_paths + glob.glob(pth+'/stack_'+str(idx+21)+'_5_3.tif')
        img_pathsE = img_paths + glob.glob(epth+'/stack_'+str(idx+20)+'_5_3.tif')
...
def get_valid_img_stack(pth, epth, num_stacks):
    ...
    for idx in range(0,num_stacks):
        img_paths = img_paths + glob.glob(pth+'/stack_'+str(idx+20)+'_5_5.tif')
        img_pathsA = img_paths + glob.glob(pth+'/stack_'+str(idx+19)+'_5_5.tif')
        img_pathsB = img_paths + glob.glob(pth+'/stack_'+str(idx+21)+'_5_5.tif')
        img_pathsE = img_paths + glob.glob(epth+'/stack_'+str(idx+20)+'_5_5.tif')
...
if __name__ == '__main__':
    ...
    model = UNet(in_channels=4, n_classes=1, depth=5, padding=True, batch_norm=True, up_mode='upsample').to(device)
    #optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0005)
    #lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.1)
    # was
    #optimizer = torch.optim.Adam(model.parameters())
    optimizer = optim.Adadelta(model.parameters(), lr=learningRate)
    scheduler = StepLR(optimizer, step_size=800, gamma=learningRateGamma)
    ...
    (trn_img_paths, trnA, trnB, trnE) = get_train_img_stack(trn_img_path, trn_edge_path, num_stacks)
    ...
    for cepoch in range(0,numEpochs):
        ...
        for trn_idx in range(0, len(trn_img_paths)):
            ...
            trn_imgA = trnA[trn_idx]
            trn_imgB = trnB[trn_idx]
            ...
            img1_a      = skio.imread(trn_imgA)
            img1_a      = img1_a[0:128, 0:128]
            img1_b      = skio.imread(trn_imgB)
            img1_b      = img1_b[0:128, 0:128]
            ...
            img1_ra = single_rotation(img1_a, prob) 
            img1_rb = single_rotation(img1_b, prob) 
            ...
            img1a = single_horizontal_flip(img1_ra, prob) 
            img1b = single_horizontal_flip(img1_rb, prob) 
            img1e = single_horizontal_flip(img1_re, prob) 

            Ysyn = np.expand_dims(seg1, axis=0)
            Xsyn = np.zeros((4, img1.shape[0], img1.shape[1]))        
            Xsyn[0,:,:] = img1
            Xsyn[1,:,:] = img1a
            Xsyn[2,:,:] = img1b
            Xsyn[3,:,:] = img1e
        ...
        if trn_cepoch_loss < trn_best_loss:
            ...
            print("\t\tCurrent best training loss", str(trn_best_loss))
            torch.save(trn_best_loss_model.state_dict(), "best_trn_loss_model.pth")
        ...
        if trn_cepoch_rerror < trn_best_rerror:
            ...
            print("\t\tCurrent best traning rand error", str(trn_best_rerror))
            torch.save(trn_best_rerror_model.state_dict(), "best_trn_rerror_model.pth")
        ...
        for val_idx in range(0, len(val_img_paths)):
            ...
            val_imgA = valA[val_idx]
            val_imgB = valB[val_idx]
            val_imgE = valE[val_idx]

            val_seg_path = val_seg_paths[val_idx]

            # Validation image
            img1      = skio.imread(val_img_path)
            img1      = img1[0:128, 0:128]
            img1a      = skio.imread(val_imgA)
            img1a      = img1a[0:128, 0:128]
            img1b      = skio.imread(val_imgB)
            img1b      = img1b[0:128, 0:128]
            img1e      = skio.imread(val_imgE)
            img1e      = img1e[0:128, 0:128]
        ...
        if val_cepoch_loss < val_best_loss:
            ...
            print("\t\tCurrent best validation loss", str(val_best_loss))
            torch.save(val_best_loss_model.state_dict(), "best_val_loss_model.pth")
        ...
        if val_cepoch_rerror < val_best_rerror:
            ...
            print("\t\tCurrent best validation rand error", str(val_best_rerror))
            torch.save(val_best_rerror_model.state_dict(), "best_val_rerror_model.pth")

    # Saving all the losses and rand_errors
    with open('trn_losses.pkl', 'wb') as f:
        pickle.dump(trn_loss_lst, f)
    with open('trn_rerrors.pkl', 'wb') as f:
        pickle.dump(trn_rerror_lst, f)
        
    with open('val_losses.pkl', 'wb') as f:
        pickle.dump(val_loss_lst, f)
    with open('val_rerrors.pkl', 'wb') as f:
