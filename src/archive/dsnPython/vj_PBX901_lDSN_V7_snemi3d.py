...
def get_train_img_stack(pth, epth, num_stacks):
    ...
    img_pathsE = []     # edge
    for idx in range(0,num_stacks):
        img_paths = img_paths + glob.glob(pth+'/stack_'+str(idx)+'_5_3.tif')
        img_pathsE = img_pathsE + glob.glob(epth+'/stack_'+str(idx)+'_5_3.tif')
...
def get_valid_img_stack(pth, epth, num_stacks):
    ...
    img_pathsE = []     # edge
    for idx in range(0,num_stacks):
        img_paths = img_paths + glob.glob(pth+'/stack_'+str(idx)+'_5_5.tif')
        img_pathsE = img_pathsE + glob.glob(epth+'/stack_'+str(idx)+'_5_5.tif')
...
if __name__ == '__main__':
    ...
    # Paths for images and segmentation labels <--- Change the paths
    ...
    for cepoch in range(0,numEpochs):
        ...
        # Bar
        bar = progressbar.ProgressBar(maxval=num_trn_imgs,\
        ...
        for trn_idx in range(0, num_trn_imgs):
        ...
        print("\t\tBest Training loss   : ", str(trn_best_loss))
        ...
        print("\t\tBest Traning error    : ", str(trn_best_rerror))
        ...
        if not(SKIP_VALIDATION):
            ...
            rerror_lst_epoch = []
            # Bar
            bar = progressbar.ProgressBar(maxval=len(val_img_paths),\
            ...
            print("\t\tBest validation loss", str(val_best_loss))
            ...
            print("\t\tBest validation rand error", str(val_best_rerror))

    # Saving all the losses and rand_errors
    with open('trn_losses_2_channel.pkl', 'wb') as f:
        pickle.dump(trn_loss_lst, f)
    with open('trn_rerrors_2_channel.pkl', 'wb') as f:
        pickle.dump(trn_rerror_lst, f)
        
    with open('val_losses_2_channel.pkl', 'wb') as f:
        pickle.dump(val_loss_lst, f)
    with open('val_rerrors_2_channel.pkl', 'wb') as f:
 ...
 #Traning error    :  0.02950925409651963
 #Validation error :  0.049092329027946274            

##Baseline
#Traning rand:  0.05953236512195378
#Validation rand:  0.05679324828304302            
