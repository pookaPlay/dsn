...
if __name__ == '__main__':
    ...
    model = UNet(in_channels=4, n_classes=1, depth=5, wf=6, padding=True, batch_norm=True, up_mode='upconv').to(device)
    
    #optimizer = torch.optim.Adam(model.parameters())
    #optimizer = torch.optim.Adamax(params, lr=learningRate, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
    #optimizer = torch.optim.SGD(model.parameters(), lr=learningRate, momentum=0.9)
    #scheduler = StepLR(optimizer, step_size=rateStep, gamma=learningRateGamma)

    optimizer = optim.Adadelta(model.parameters(), rho=0.99, lr=learningRate)
    ...
    for cepoch in range(0,numEpochs):
        ...
        with torch.enable_grad():                            
            ...
            if trn_cepoch_loss < trn_best_loss:
                ...
                #print("\t\tBest Loss   : ", str(trn_best_loss))                
                #print("\t\tBest Error    : ", str(trn_best_rerror))                
            ...
            if ((cepoch % 500) == 0): 
                print("Saving scores so far")
                vname = 'trn_loss_2_' + str(cepoch) + '.pkl'
                with open(vname, 'wb') as f:
                    pickle.dump(trn_loss_lst, f)
                tname = 'trn_error_2_' + str(cepoch) + '.pkl'                    
    ...
    print("\nFinal best training loss", str(trn_best_loss) + " at " + str(trn_best_loss_epoch))
    print("Final best traning rand error", str(trn_best_rerror) + " at " + str(trn_best_rerror_epoch))
