...
if __name__ == '__main__':
    ...
    for cepoch in range(0,numEpochs):
        ...
        with torch.enable_grad():                            
            ...
            loss_lst_epoch   = []
            rerror_lst_epoch = []
            ti = [794, 240, 389, 14, 303, 436, 55, 40]
            for trn_idx in range(0, numTrain):
                ...
                print("\t\tLoss   : " + str(loss.detach().numpy()) + "   Error: " + str(randError) + " for image: " + str(ti[trn_idx]) + "\n")
            ...
            if (cepoch % logScores)==0:
                vname = "train1D3_model_" + str(cepoch) + ".pth"
                torch.save(model.state_dict(), vname)            
                vname = "train1D3_loss_" + str(cepoch) + ".pkl"                                
                with open(vname, 'wb') as f:
                    pickle.dump(trn_loss_lst, f)            
                vname = "train1D3_error_" + str(cepoch) + ".pkl"                                
                with open(vname, 'wb') as f:
